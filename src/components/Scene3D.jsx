...
function CameraController() {
  ...
  useFrame(() => {
    ...
    const mouseX = config.enableMouseParallax ? (window.mouseX || 0) * 0.45 : 0;
    const mouseY = config.enableMouseParallax ? (window.mouseY || 0) * 0.45 : 0;
...
function DustParticles({ count = 350 }) {
  ...
  // Clean up texture to avoid leaks
  useEffect(() => {
    return () => {
      glowingCircleTexture.dispose();
    };
  }, [glowingCircleTexture]);
...
export function Scene3D({ activeSoftware = 'resolve' }) {
  ...
  const frameloop = (tabVisible && inViewport) ? 'always' : 'never';
  ...
  return (
    <div ref={containerRef} className="canvas-container">
      ...
      >
        ...
        {config.showFloatingLogos && <FloatingFrames />}
        <CinemaScreen />
        {config.showWorkspace && <Workspace3D activeSoftware={activeSoftware} />}
        <DustParticles count={config.particleCount} />
