...
function drawScopeScreen(canvas, software, time) {
  ...
  if (software === 'resolve') {
    ...
    ctx.beginPath();
    for (let x = 0; x < w; x++) {
      const y = 180 + Math.sin(x * 0.05 + time * 3) * 20 + Math.cos(x * 0.15) * 10 + (Math.random() - 0.5) * 8;
    ...
    ctx.beginPath();
    for (let x = 0; x < w; x++) {
      const y = 210 + Math.cos(x * 0.06 + time * 2.5) * 25 + Math.sin(x * 0.22) * 8 + (Math.random() - 0.5) * 8;
      if (x === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
    }
    ctx.stroke();
    ...
    ctx.beginPath();
    for (let x = 0; x < w; x++) {
      const y = 240 + Math.sin(x * 0.07 + time * 4) * 15 + Math.cos(x * 0.12) * 12 + (Math.random() - 0.5) * 8;
      if (x === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
    }
    ctx.stroke();
    ...
    for (let i = 0; i < 4; i++) {
      const bx = 100 + i * 80;
      ctx.fillStyle = '#101015';
      ctx.fillRect(bx, 60, 30, 260);
      const hVal = 100 + Math.sin(time * 5 + i * 2.5) * 80 + Math.cos(time * 8) * 15;
      const grad = ctx.createLinearGradient(bx, 320, bx, 60);
      grad.addColorStop(0, '#22c55e');
      grad.addColorStop(0.65, '#eab308');
      grad.addColorStop(1, '#ef4444');
      ctx.fillStyle = grad;
      ctx.fillRect(bx, 320 - hVal, 30, hVal);
    }
    ...
    ctx.beginPath();
    for (let x = 0; x < w; x++) {
      const y = 180 + Math.sin(x * 0.08 + time * 4.5) * 35 * Math.sin(x * 0.01) + (Math.random() - 0.5) * 4;
      if (x === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);
    }
    ctx.stroke();
    
    ctx.fillStyle = 'rgba(214, 137, 250, 0.15)';
    ctx.fillRect(40, 240, 432, 40);
    ctx.fillStyle = '#d689fa';
    const cacheProgress = (time * 25) % 432;
    ctx.fillRect(40, 240, cacheProgress, 40);
    ctx.fillStyle = '#ffffff';
    ctx.fillText(`Caching frame ${Math.floor(time * 24) % 120} / 120...`, 50, 265);
